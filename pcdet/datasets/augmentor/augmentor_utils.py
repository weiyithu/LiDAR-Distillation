...
def random_flip_along_x(gt_boxes, points, enable_gt=None):
    """
    Args:
...
def random_flip_along_y(gt_boxes, points, enable_gt=None):
    """
    Args:
        gt_boxes: (N, 7 + C), [x, y, z, dx, dy, dz, heading, [vx], [vy]]
        points: (M, 3 + C)
...
def global_rotation(gt_boxes, points, rot_range, noise_rotation_gt=None):
    """
    Args:
        gt_boxes: (N, 7 + C), [x, y, z, dx, dy, dz, heading, [vx], [vy]]
        points: (M, 3 + C),
    ...
    Returns:
    """
    if noise_rotation_gt is None:
        noise_rotation = np.random.uniform(rot_range[0], rot_range[1])
    else:
        noise_rotation = noise_rotation_gt
...
def global_scaling(gt_boxes, points, scale_range, noise_scale_gt=None):
    """
    Args:
    ...
    if noise_scale_gt is None:
        noise_scale = np.random.uniform(scale_range[0], scale_range[1])
    else:
        noise_scale = noise_scale_gt
